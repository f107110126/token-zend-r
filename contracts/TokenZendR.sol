...
contract TokenZendR is Ownable, Pausable {
     ...
     * @param contract_ contract address of ERC20 token to transfer
    ...
    /**
     * @dev a mapping from transaction ID's the sender address
     * that initiates them. Owners can create serveral transactions
     */
    ...
    function transferTokens(bytes32 symbol_, address to_, uint256 amount_) public whenNotPaused {
        ...
        if(amount_ > ERC20Interface.allowance(from_, address(this))) {
            ...
            revert('Transaction is not allowed.');
        }
     ...
     */
    function withdraw(address payable beneficiary) public onlyOwner whenNotPaused {
